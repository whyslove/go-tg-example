...
func (http *httpMathematical) serveAdd(ctx *fiber.Ctx) (err error) {
	return http.serveMethod(ctx, "add", http.add)
}
func (http *httpMathematical) add(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {
	var err error
	var request requestMathematicalAdd
	...
	response.Result, err = http.svc.Add(methodContext, request.A, request.B)
	if err != nil {
...
func (http *httpMathematical) serveBatch(ctx *fiber.Ctx) (err error) {
	...
	for _, request := range requests {
		...
		case "add":
			...
			func(request baseJsonRPC) {
				responses.append(http.add(ctx, request))
				wg.Done()
			}(request)
		default:
...
func (http *httpMathematical) serveMethod(ctx *fiber.Ctx, methodName string, methodHandler methodJsonRPC) (err error) {
	methodHTTP := ctx.Method()
	if methodHTTP != fiber.MethodPost {
		ctx.Response().SetStatusCode(fiber.StatusMethodNotAllowed)
		if _, err = ctx.WriteString("only POST method supported"); err != nil {
